    ...
    protected function grid()
    {
        ...
        $grid->header(
            function ($type) { 
                ...
                $year = $now->format('Y');
                $mon = $now->format('m');
                $lastmon = $now->modify('-1 months')->format('m');
                if ($mon == $lastmon) {
                    $lastmon -= 1;
                    $lastmon = str_pad($lastmon, 2, '0', STR_PAD_LEFT);
                }
                $lastmon1 = $now->modify('-1 months')->format('m');
                if ($lastmon1 == $lastmon) {
                    $lastmon1 -= 1;
                    $lastmon1 = str_pad($lastmon1, 2, '0', STR_PAD_LEFT);
                }
                $lastmon2 = $now->modify('-1 months')->format('m');
                if ($lastmon2 == $lastmon1) {
                    $lastmon2 -= 1;
                    $lastmon2 = str_pad($lastmon2, 2, '0', STR_PAD_LEFT);
                }
